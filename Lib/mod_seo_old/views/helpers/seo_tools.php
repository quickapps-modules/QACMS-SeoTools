<?php
class SeoToolsHelper extends Helper {
	
	var $red = 0;
	var $yellow = 0;
	var $green = 0;
	
	var $essential_passed = 0;
	var $essential_failed = 0;
	
	var $very_important_passed = 0;
	var $very_important_failed = 0;
	
	var $important_passed = 0;
	var $important_failed = 0;
	
	var $tmp = null;
	
	function competitor_average($data = array(), $type = 'integer' ){
		...
		if ( $type == 'integer')
			return sprintf(_e('%s to %s'), number_format($min, 2), number_format($max, 2)); 
	...
	function advice_color($competitor = array(), $your, $aspect = null){
		if ( $aspect === null)
			return false;
	...
	function __str2int($data){
		if ( is_array($data) ){
			foreach( $data as &$rank){ $rank = (int)str_replace(array(',', '.'), '', $rank); }
	...
	function emphasize_keywords($str, $words = array(), $type = 'bold'){
	
		switch($type){
			case 'highlight': default: $var_style = 'background: #CEDAEB;'; break;
			case 'bold': $var_style = 'font-weight: bold;'; break;
			case 'underline': $var_style = 'text-decoration: underline;'; break;
		}
		...
		if( is_array($words) ) {
			foreach($words as $word){
				...
				if( !empty($word) ){
					...
					"<span style='".$var_style."'>\\1</span>", $str);
				}
			}
	...
	function return_color($num, $aspect = null){
	...
	function age($data){
		...
		$out = sprintf(_e('%s years, %s days. (%s)'), $data['years'], $data['days'], date(_e('Y-m-d'), strtotime(date('Y-m-d'))-$stamp) );
		return $out;
	}
	
	function color_totals(){
		return array(
			'red' => $this->red,
			'yellow' => $this->yellow,
			'green' => $this->green
