<?php
class PageRankComponent extends Component {
    public function main(Controller $Controller) {
	        if (!isset($Controller->data['Tool']['url'])) {
            return false;
        }

		return $this->getpageRank($Controller->data['Tool']['url']);
    }
	...
	private function __hashURL($String) {
	...
	private function __checkHash($Hashnum) {
		$CheckByte = 0;
		$Flag = 0;
